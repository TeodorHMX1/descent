	...
	public class CameraDistanceRaycaster : MonoBehaviour
	{
		...
		void LateUpdate()
		{
			...
			cameraTransform.position =
				tr.position + (cameraTargetTransform.position - tr.position).normalized * currentDistance;
		}
		...
		float GetCameraDistance()
		{
			...
			{
				//Cast ray;
				if (Physics.Raycast(new Ray(tr.position, _castDirection), out _hit,
					_castDirection.magnitude + minimumDistanceFromObstacles, layerMask, QueryTriggerInteraction.Ignore))
				{
			...
			{
				//Cast sphere;
				if (Physics.SphereCast(new Ray(tr.position, _castDirection), spherecastRadius, out _hit,
					_castDirection.magnitude, layerMask, QueryTriggerInteraction.Ignore))
				{
					//Return distance;
					return _hit.distance;
				}
