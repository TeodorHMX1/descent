...
namespace ZeoFlow.PlayerMovement
{
	...
	public class CameraController : MonoBehaviour
	{
		...
		protected void RotateCamera(float _newHorizontalInput, float _newVerticalInput)
		{
			...
			{
				//Lerp input;
				oldHorizontalInput = Mathf.Lerp(oldHorizontalInput, _newHorizontalInput,
					Time.deltaTime * cameraSmoothingFactor);
				oldVerticalInput = Mathf.Lerp(oldVerticalInput, _newVerticalInput,
					Time.deltaTime * cameraSmoothingFactor);
			}
			else
			{
		...
		public void RotateTowardDirection(Vector3 _direction, float _lookSpeed)
		{
			...
			float _xAngleDifference = VectorMath.GetAngle(new Vector3(0f, _currentLookVector.y, 1f),
				new Vector3(0f, _direction.y, 1f), Vector3.right);
