	...
	public class Sensor
	{
		...
		private void CastRayArray(Vector3 _origin, Vector3 _direction)
		{
			...
			for (int i = 0; i < raycastArrayStartPositions.Length; i++)
			{
				...
				if (Physics.Raycast(_rayStartPosition, rayDirection, out _hit, castLength, layermask,
		...
		{
			RaycastHit _hit;
			hasDetectedHit = Physics.Raycast(_origin, _direction, out _hit, castLength, layermask,
				QueryTriggerInteraction.Ignore);
		...
		{
			RaycastHit _hit;
			hasDetectedHit = Physics.SphereCast(_origin, sphereCastRadius, _direction, out _hit,
		...
		public void DrawDebug()
		{
			if (hasDetectedHit && isInDebugMode)
			{
				...
				Debug.DrawLine(hitPosition + Vector3.up * _markerSize, hitPosition - Vector3.up * _markerSize,
					Color.green, Time.deltaTime);
				Debug.DrawLine(hitPosition + Vector3.right * _markerSize, hitPosition - Vector3.right * _markerSize,
					Color.green, Time.deltaTime);
				Debug.DrawLine(hitPosition + Vector3.forward * _markerSize, hitPosition - Vector3.forward * _markerSize,
					Color.green, Time.deltaTime);
			}
		}
		...
		{
			raycastArrayStartPositions =
				GetRaycastStartPositions(ArrayRows, arrayRayCount, offsetArrayRows, sphereCastRadius);
		}
	}
}