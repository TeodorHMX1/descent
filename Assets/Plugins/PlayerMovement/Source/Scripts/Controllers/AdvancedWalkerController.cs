...
namespace ZeoFlow.PlayerMovement
{
	...
	public class AdvancedWalkerController : Controller
	{
		...
		//Run key variables;
		bool runInputIsLocked = false;
		bool runKeyWasPressed = false;
		bool runKeyWasLetGo = false;
		bool runKeyIsPressed = false;
		...
		{
			float speedVelocity = walkSpeed;
			if (canRun && runKeyIsPressed)
			{
				speedVelocity = runSpeed;
			}
		...
		void HandleJumping()
		{
			if (currentControllerState == ControllerState.Grounded)
			{
				if (canJump && (jumpKeyIsPressed == true || jumpKeyWasPressed) && !jumpInputIsLocked)
				{
					//Call events;
					OnGroundContactLost();
					OnJumpStart();
		...
		void OnGroundContactLost()
		{
			...
			Vector3 _currentVelocity = GetMomentum() + Vector3.ClampMagnitude(savedMovementVelocity,
				Mathf.Clamp(speedVelocity - _horizontalMomentumSpeed, 0f, speedVelocity));
			...
			//Subtract from '_length', based on 'speedVelocity' and 'airControl', check for overshooting;
			if (_length >= speedVelocity * airControl)
				_length -= speedVelocity * airControl;
			else
				_length = 0f;
