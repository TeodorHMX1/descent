...
namespace Destructible
{
	public static class ExplosionHelper
	{
		...
		{
			RaycastHit hit;
			if (Physics.Raycast(explosion.Position, (gameObj.transform.position - explosion.Position), out hit,
				explosion.Radius))
			{
		...
		public static void ApplyForcesToDebris<T>(GameObject destroyedObj, float velocityReduction, T damageInfo)
		{
			...
			if (damageInfo.GetType() == typeof(ExplosiveDamage))
			{
				...
				foreach (Rigidbody debrisRigidbody in debrisRigidbodies)
					debrisRigidbody.AddExplosionForce(explosion.BlastForce, explosion.Position, explosion.Radius,
			...
			if (damageInfo.GetType() == typeof(ImpactDamage))
			{
				...
				{
					foreach (Rigidbody debrisRigidbody in debrisRigidbodies)
						debrisRigidbody.AddExplosionForce(impact.AdditionalForce, impact.AdditionalForcePosition,
							impact.AdditionalForceRadius, 0f);
				}
			}
		}
	}
