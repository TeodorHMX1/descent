...
namespace Destructible
{
	public static class DestructionExtensions
	{
		...
		public static void MakeDebrisCling(this GameObject destroyedObj)
		{
			...
			for (int i = 0; i < clingPoints.Length; i++)
			{
				...
				if (clingPoints[i].chanceToCling < 100) // 100% chance always clings
				{
					...
					if (randomNbr > clingPoints[i].chanceToCling
					) // exit if random number is outside the possible chance.
						continue;
				}

				// Check if there's anything to cling to.
				Ray ray = new Ray(clingPoints[i].transform.position - (clingPoints[i].transform.forward * 0.025f),
					clingPoints[i].transform.forward); // need to start the ray behind the transform a little
				RaycastHit hitInfo;
				if (Physics.Raycast(ray, out hitInfo, 0.075f))
				{
					...
					if (clingPointRbody.gameObject.GetComponent<Destructible>() &&
						!hitInfo.collider.gameObject.GetComponent<DestructibleParent>())
					...
					ClingPoint[] clingPointsToDestroy =
						clingPointRbody.gameObject.GetComponentsInChildren<ClingPoint>();
		...
		public static void ProcessDestructibleCollision(this Destructible destructibleObj, Collision collision,
			Rigidbody collidingRigidbody)
		{
			...
			if (otherRbody != null)
			{
				...
				impactDamage = Vector3.Dot(collision.contacts[0].normal, collision.relativeVelocity) *
							   collidingRigidbody.mass;
			...
			if (impactDamage > 1f) // impact must do at least 1 damage to bother with.
			{
				...
				ImpactDamage impactInfo = new ImpactDamage()
				{
					ImpactObject = otherRbody, DamageAmount = (int) impactDamage,
					ImpactObjectVelocityFrom = collision.relativeVelocity * -1
				};
		...
		public static void CalculateDamageLevels(this List<DamageLevel> damageLevels, float maxHitPoints)
		{
			...
			for (int i = 0; i < damageLevels.Count; i++)
			{
				...
				{
					damageLevels[i].maxHitPoints =
						Mathf.RoundToInt(maxHitPoints * (.01f * damageLevels[i].healthPercent));
					damageLevels[i - 1].minHitPoints =
						Mathf.RoundToInt(maxHitPoints * (.01f * damageLevels[i].healthPercent)) + 1;
				}
