...
namespace Destructible
{
	...
	public class ObjectPool : MonoBehaviour
	{
		...
		private Dictionary<int, GameObject>
			autoPooledObjects; // Destroyed Prefabs which are auto-added to the pool on start.
		...
		private static void CreateInstance()
		{
			...
			if (objectPools.Length == 0)
				Debug.LogError(
					"ObjectPool script not found in scene. This is required for Destructible to work properly.");
		...
		public GameObject Spawn(GameObject originalPrefab, Vector3 position, Quaternion rotation, Transform parent,
			int autoPoolID = 0)
		{
			...
			for (int i = 0; i < prefabsToPool.Count; i++)
			{
				...
				if (Pool != null && Pool[i].Length > 0)
				{
					...
					for (int j = 0; j < Pool[i].Length; j++)
					{
						if (Pool[i][j] != null)
						{
							...
							if (parent != null
							) // If a parent transform was specified, set the position local to the parent.
								pooledObj.transform.localPosition = position;
							else // Otherwise, set the position in relation to world space.
								pooledObj.transform.position = position;
				...
				if (Pool == null)
				{
					...
					Debug.LogWarning("[" + origPrefabName +
									 " was instantiated instead of spawned from pool. Reason: Pool is null.");
					return pooledObj;
				}
				...
				if (!prefabsToPool[i].OnlyPooled)
				{
					...
					if (!suppressWarnings)
						Debug.LogWarning("[" + origPrefabName +
										 " was instantiated instead of spawned from pool. Reason: No objects remaining in the pool (size: " +
										 Pool[i].Length + "). Consider increasing the pool size.");
					return pooledObj;
				}
			...
			if (!suppressWarnings)
				Debug.LogWarning("[" + origPrefabName +
								 "] was instantiated instead of spawned from pool. Reason: Prefab not found in pool.");
		...
		public void PoolObject(GameObject obj, bool reenableChildren = false)
		{
			for (int i = 0; i < prefabsToPool.Count; i++)
			{
				...
				if (!suppressWarnings)
					Debug.LogWarning("[" + obj.name +
									 "] was destroyed instead of pooled. Reason: The pool size for this prefab was too small (" +
									 Pool[i].Length + "). Consider increasing the pool size.");
			...
			if (!suppressWarnings)
				Debug.LogWarning(
					"[" + obj.name + "] was destroyed instead of pooled. Reason: Prefab not found in pool.");
		}
	}
}