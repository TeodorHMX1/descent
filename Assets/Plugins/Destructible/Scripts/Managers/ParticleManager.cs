...
namespace Destructible
{
	...
	public class ParticleManager : MonoBehaviour
	{
		...
		public int
			maxPerDestructible = 5; // max particles to allow for a single Destructible object or DestructibleGroup.
			...
			bool transferSourceMat)
		{
			...
			ActiveParticle aParticle = new ActiveParticle()
				{GameObject = spawn, InstantiatedTime = Time.time, ParentId = parentId};
			...
			foreach (ParticleSystemRenderer psr in spawn.GetComponentsInChildren<ParticleSystemRenderer>())
			{
				...
				psr.material = destObj.fallbackParticleMaterial != null
					? destObj.fallbackParticleMaterial
					: destObj.GetDestroyedParticleEffectMaterial();
				Texture2D detailMask = DestructionManager.Instance.GetDetailMask(psr.sharedMaterial,
					destObj.damageLevels[destObj.damageLevels.Count - 1]);
