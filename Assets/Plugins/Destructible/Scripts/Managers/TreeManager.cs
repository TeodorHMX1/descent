...
namespace Destructible
{
	...
	public class TreeManager : MonoBehaviour
	{
		...
		[Tooltip(
			"Backs up the active terrain in the editor when you play the scene. This way if a crash occurs, you'll be able to restore from the backup and won't lose your placed trees, since the TreeManager replaces terrain trees with destructible stand-ins at runtime.")]
		...
		private List<TreeInstance>
			currentTreeInstances; // NOTE: It's important to keep this a List, don't convert to Array

		private TreeInstance[] originalTreeInstances;

		private bool
			isTerrainDataDirty; // Determines if the terrainData has a backup that hasn't been resolved (possibly from a Unity crash). Don't try to make changes to a dirty TerrainData - give the user an option to fix it first.
		...
		private void Start()
		{
			...
			if (terrainDataBkp != null)
			{
				...
				Debug.LogError("Cannot backup terrainData for [" + terrain.terrainData.name +
							   "]. A backup already exists. Please exit Play mode to fix.");
				return;
			}
			#endif

			if (treeInstances == null || treeInstances.Length == 0 || treePrototypes == null ||
			...
			{
				Debug.LogWarning(
					"Tree prototypes do not match Destructible's tree stand-in prefabs. Please click the \"Update Trees\" button on the TreeManager script.");
				return;
			}

			for (int i = 0; i < treePrototypes.Length; i++)
			{
				if (destructibleTrees[i].Prefab == null || treePrototypes[i].prefab == null ||
					treePrototypes[i].prefab.name != destructibleTrees[i].Prefab.name)
				{
					Debug.LogWarning(
						"Tree prototype names do not match Destructible tree stand-in prefab names. You may need to click the \"Update Trees\" button on the TreeManager script.");
					return;
				}
			}
			...
			for (int i = 0; i < treeInstances.Length; i++)
			{
				...
				DestructibleTree destructibleTree =
					destructibleTrees.Find(x => x.prototypeIndex == tree.prototypeIndex);
		...
		public void UpdateTrees()
		{
			...
			for (int i = 0; i < terrain.terrainData.treePrototypes.Length; i++)
			{
				...
				{
					if (comp.GetType() != typeof(Transform) && comp.GetType() != typeof(CapsuleCollider) &&
						comp.GetType() != typeof(BoxCollider) &&
						comp.GetType() != typeof(SphereCollider) && comp.GetType() != typeof(MeshCollider) &&
						comp.GetType() != typeof(Destructible) &&
						comp.GetType() != typeof(HitEffects) && comp.GetType() != typeof(TagIt) &&
						comp.GetType() != typeof(ParticleSystem) &&
						comp.GetType() != typeof(ParticleSystemRenderer) &&
						comp.GetType() != typeof(WhenDestroyedResetTree))
		...
		public void RestoreTrees()
		{
			...
			if (isTerrainDataDirty)
			{
				...
				Debug.LogWarning(
					"TerrainData is dirty (there is a backup that has not been resolved). Exiting restore process to prevent overwriting.");
				return;
			}
			#endif
