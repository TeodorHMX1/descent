...
namespace Destructible
{
	...
	{
		[Tooltip(
			"If true, Destructible scripts will be deactivated on start, and will activate any time they are inside a trigger collider with the ActivateDestructibles script on it.")]
		[HideInInspector]
		public bool autoDeactivateDestructibles;

		[Tooltip(
			"If true, Destructible terrain object scripts will be deactivated on start, and will activate any time they are inside a trigger collider with the ActivateDestructibles script on it.")]
		[HideInInspector]
		public bool autoDeactivateDestructibleTerrainObjects = true;

		[Tooltip(
			"If true, Destructible terrain tree scripts will not be activated by ActivateDestructibles scripts. Recommended to leave this true for performance, unless you need to move trees during the game or use progressive damage textures on them.")]
		[HideInInspector]
		public bool destructibleTreesStayDeactivated = true;

		[Tooltip(
			"The time in seconds to automatically deactivate Destructible scripts when they are outside an ActivateDestructibles trigger area.")]
		[HideInInspector]
		...
		[Tooltip(
			"Maximum allowed destroyed prefabs within [withinSeconds] seconds. When this limit is reached, a particle effect will be used instead.")]
		public int destroyedPrefabLimit = 15;

		[Tooltip(
			"Number of seconds within which no more than [destroyedPrefabLimit] destructions will be instantiated.")]
		...
		[Tooltip(
			"Anything larger than this (in game units) is considered a large object, and will get the default large particle.")]
		public float smallObjectMaxSize = 1f;

		[Tooltip(
			"If damage done to an object is more than this many times its hit points, it will be obliterated into a particle effect if CanBeObliterated=true.")]
		...
		[HideInInspector]
		public bool
			useCameraDistanceLimit =
				true; // If true, things beyond the specified distance from the main camera will be destroyed in a more limiting (ie, higher performance) way.

		[HideInInspector]
		public int
			cameraDistanceLimit =
				100; // Specified game units (usually meters) from camera, where destruction limiting will occur.

		[HideInInspector] public int debrisLayer = -1;

		[HideInInspector]
		public Collider[]
			overlapColliders; // These are the colliders overlapped by an Overlap Sphere (used for determining affected objects in a blast radius without allocating GC).
		...
		private void Awake()
		{
			...
			if (Check.LayerExists("DestructibleDebris", false) == false)
				Debug.LogWarning(
					"DestructibleDebris layer not found. Add a layer named 'DestructibleDebris' to your project if you want debris to ignore other debris when using Cling Points.");
		}
			...
			bool isObliterated)
		{
			...
			if (oldObj.damageEffects != null)
			{
				foreach (var damageEffect in oldObj.damageEffects)
				{
					...
					if (!damageEffect.StopEmittingOnDestroy || damageEffect.ParticleSystems == null ||
						damageEffect.ParticleSystems.Length <= 0) continue;
			...
			GameObject newObj = ObjectPool.Instance.Spawn(destroyedPrefab, oldObj.PositionFixedUpdate,
				oldObj.RotationFixedUpdate, oldObj.GetInstanceID());
		...
		private void DestroyWithParticleEffect<T>(Destructible oldObj, ParticleSystem customParticle, T damageInfo)
		{
			if (oldObj.useFallbackParticle)
			{
				...
				{
					ParticleManager.Instance.PlayEffect(customParticle, oldObj, position, oldObj.transform.rotation,
						instanceId, false);
				}
				else // Otherwise, if no specific fallback particle effect is defined, use a default particle effect assigned in DestructionManager.
				{
					...
					ParticleManager.Instance.PlayEffect(defaultParticle, oldObj, position, oldObj.transform.rotation,
						instanceId, true);
				}
			}
		...
		private void InstantiateDebris<T>(GameObject newObj, Destructible oldObj, T damageInfo)
		{
			...
			) // if the old object was autopooled, the destroyed object will come from the pool already having the right materials on it.
			...
			) // if destructible object has been scaled in the scene
			...
			if (debrisRigidbodies.Length > 0)
			{
				for (int i = 0; i < debrisRigidbodies.Length; i++)
				{
					...
					if (oldObj.debrisToReParentByName != null && oldObj.debrisToReParentByName.Count > 0 &&
						oldObj.transform.parent != null && (oldObj.debrisToReParentByName.Contains("ALL DEBRIS") ||
															oldObj.debrisToReParentByName.Contains(debrisRigidbodies[i]
																.name)))
					{
