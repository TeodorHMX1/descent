...
namespace Destructible
{
	...
	{
		[Range(0f, 60f)] public float delaySeconds = 10f; // How long to delay before starting the fade.
		[Range(0f, 60f)] public float fadeSeconds = 2f; // How long to spend fading out the particle effect.

		[Range(1, 30)]
		public int
			updatesPerSecond =
				15; // How often to update the fade effect. Higher values have smoother fade-out, but higher performance cost.
		...
		void Fade()
		{
			...
			if (particleEffectProperties.Count == 0)
			{
				...
				foreach (ParticleSystem particleSystem in particleSystems)
				{
					...
					particleEffectProperties.Add(new ParticleEffectPropertyBag()
						{ParticleSystem = particleSystem, TintColorStart = tintColorStart});
				}
			}
			...
			foreach (ParticleEffectPropertyBag particleSystemPropBag in particleEffectProperties)
			{
				...
				float newAlphaValue =
					Mathf.Clamp01(particleSystemPropBag.TintColorStart.a - (fadeAmountPerStep * stepCounter));
