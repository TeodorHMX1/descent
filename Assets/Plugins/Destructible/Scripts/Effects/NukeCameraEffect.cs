...
namespace Destructible
{
	...
	public class NukeCameraEffect : MonoBehaviour
	{
		...
		[Tooltip(
			"Alternate tonemapping settings to make the scene look over-exposed and strange while the nuke is active.")]
		public Tonemapping nukeTonemapping;

		[Tooltip(
			"Optional additional color correction curves to use on the main camera to make the scene look strange while the nuke is active.")]
		...
		public void OnNukeEnd()
		{
			...
			if (regularTonemapping != null && !regularTonemapping.enabled && nukeTonemapping != null &&
				nukeTonemapping.enabled)
			{
				nukeTonemapping.enabled = false;
