...
namespace Destructible
{
	...
	{
		public GameObject
			playerDustPrefab; // A dust cloud particle effect that surrounds the player when the shockwave hits.

		public float
			dustDurationSeconds =
				10f; // How long (in seconds) to play the dust effect around a player after being hit by the shockwave.
			...
			if (collider.tag == "Player")
			{
				if (playerDustPrefab != null)
				{
					...
					GameObject dustCloud =
						Instantiate(playerDustPrefab, player.position, Quaternion.identity) as GameObject;
