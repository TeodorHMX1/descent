...
namespace Destructible
{
	...
	public class DestructibleEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			// TOTAL HIT POINTS
			EditorGUILayout.PropertyField(totalHitPoints,
				new GUIContent("Total Hit Points", "The maximum, or starting hit points, of the object."));
			if (totalHitPoints.floatValue < 0)
				totalHitPoints.floatValue = 0;
			if (totalHitPoints.floatValue != destructible.totalHitPoints
			) // value has changed, so change the current hit points to match.
				currentHitPoints.floatValue = totalHitPoints.floatValue;
			destructible.totalHitPoints = totalHitPoints.floatValue;

			// CURRENT HIT POINTS
			EditorGUILayout.PropertyField(currentHitPoints,
				new GUIContent("Current Hit Points", "The current hit points of the object."));
			...
			// CAN BE DESTROYED
			canBeDestroyed.boolValue = EditorGUILayout.Toggle(
				new GUIContent("Can Be Destroyed",
					"If checked, the object can be damaged until it is destroyed. If unchecked, the object can still be damaged and will display any progressive damage and damage effects, but cannot be destroyed."),
				canBeDestroyed.boolValue);
			...
			if (destructible.canBeDestroyed)
			{
				...
				canBeObliterated.boolValue = EditorGUILayout.Toggle(
					new GUIContent("Can Be Obliterated",
						"If checked, this object will be obliterated into the fallback particle effect when it receives excessive damage (defined by the Obliterate Modifier on the Destruction Manager script)."),
					canBeObliterated.boolValue);
			...
			// CAN BE REPAIRED
			canBeRepaired.boolValue =
				EditorGUILayout.Toggle(
					new GUIContent("Can Be Repaired",
						"If checked, this object is capable of being repaired. If unchecked, this object will ignore any attempt to repair it."),
					canBeRepaired.boolValue);
			...
			{
				GUILayout.BeginHorizontal();
				sinkWhenDestroyed.boolValue = EditorGUILayout.Toggle(
					new GUIContent("Sink on Destroy",
						"If checked, this object will sink into the ground when destroyed, instead of emitting a particle effect or swapping to a destroyed prefab. Requires a rigidbody on the object."),
					sinkWhenDestroyed.boolValue);
				GUILayout.EndHorizontal();
			...
			// VELOCITY REDUCTION
			destructible.velocityReduction = EditorGUILayout.Slider(
				new GUIContent("Velocity Reduction",
					"How much this object reduces the velocity of fast-moving objects that impact and destroy it."),
				destructible.velocityReduction, 0f, 1f);

			// IGNORE COLLISIONS UNDER
			GUILayout.BeginHorizontal();
			GUILayout.Label(
				new GUIContent("Ignore Collisions Under",
					"Collisions with this object will be ignored if they are under this magnitude. Useful for things like tires, which constantly have small-to-medium collisions with the terrain."),
				GUILayout.Width(140));
			...
			foreach (Renderer rend in renderers)
			{
				foreach (Material mat in rend.sharedMaterials)
				{
					...
					if (mat.HasProperty("_DetailMask") &&
						mat.HasProperty("_DetailAlbedoMap") && mat.HasProperty("_DetailNormalMap") &&
						mat.GetTexture("_DetailMask") != null &&
			...
			GUIContent visibleDamageContent = new GUIContent("Visible Damage",
				"Visible Damage Levels require using the Standard shader for your materials. You also need to use the Detail Mask to control visibility, and both Secondary Maps to add damage.");
			...
			string[] popupDisplay =
			{
				"0 Undamaged", "1 Light", "2 Light", "3 Light", "4 Medium", "5 Medium", "6 Medium", "7 Heavy",
				"8 Heavy", "9 Heavy"
			};
			...
			for (int i = 0; i < destructible.damageLevels.Count; i++)
			{
				...
				if (i > 0)
				{
					if (destructible.damageLevels[i].hasError)
						GUI.color = Color.red;
					destructible.damageLevels[i].healthPercent =
				...
				EditorGUILayout.LabelField(
					destructible.damageLevels[i].maxHitPoints + "-" + destructible.damageLevels[i].minHitPoints,
					GUILayout.Width(66));
				if (!destructible.UseProgressiveDamage)
					GUI.enabled = false;
				destructible.damageLevels[i].visibleDamageLevel =
					EditorGUILayout.IntPopup(destructible.damageLevels[i].visibleDamageLevel, popupDisplay,
						popupValues);
				GUI.enabled = true;
				EditorGUILayout.EndHorizontal();
			...
			if (showAddDamageLevel && GUILayout.Button("+", EditorStyles.toolbarButton, GUILayout.Width(30)))
				destructible.damageLevels.Add(new DamageLevel
					{healthPercent = Mathf.RoundToInt(lowestHealthPercent / 2f)});
			...
			foreach (DamageEffect damageParticle in destructible.damageEffects)
			{
				...
				damageParticle.Prefab =
					EditorGUILayout.ObjectField(damageParticle.Prefab, typeof(GameObject), false) as GameObject;
				EditorGUILayout.EndHorizontal();
				...
				damageParticle.Offset = EditorGUILayout.Vector3Field("", damageParticle.Offset, GUILayout.Width(142),
					GUILayout.Height(18));
				EditorGUILayout.EndHorizontal();

				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.LabelField("Rotate", GUILayout.Width(40));
				damageParticle.Rotation = EditorGUILayout.Vector3Field("", damageParticle.Rotation,
					GUILayout.Width(142), GUILayout.Height(18));
				EditorGUILayout.EndHorizontal();

				EditorGUILayout.BeginHorizontal();
				damageParticle.HasTagDependency =
					EditorGUILayout.Toggle(damageParticle.HasTagDependency, GUILayout.Width(15));
				...
				EditorGUILayout.BeginHorizontal();
				damageParticle.UnparentOnDestroy =
					EditorGUILayout.Toggle(damageParticle.UnparentOnDestroy, GUILayout.Width(15));
				EditorGUILayout.LabelField(
					new GUIContent("Unparent when destroyed",
						"If checked, effect will be unparented when the object is destroyed. If unchecked, effect will be destroyed immediately with the object, which will stop any emissions abruptly."),
					GUILayout.Width(180));
				EditorGUILayout.EndHorizontal();
				...
				{
					EditorGUILayout.BeginHorizontal();
					damageParticle.StopEmittingOnDestroy =
						EditorGUILayout.Toggle(damageParticle.StopEmittingOnDestroy, GUILayout.Width(15));
					EditorGUILayout.LabelField(
						new GUIContent("Stop emitting when destroyed",
							"If checked, effect will stop emitting particles and be disabled when object is destroyed, allowing particles to disperse over time."),
						GUILayout.Width(180));
					EditorGUILayout.EndHorizontal();
				}
			...
			destructible.destroyedPrefab =
				EditorGUILayout.ObjectField(destructible.destroyedPrefab, typeof(GameObject), false) as GameObject;
			...
			if (destructible.destroyedPrefab != null)
			{
				...
				List<Renderer> destroyedMeshes =
					destructible.destroyedPrefab.GetComponentsInChildren<Renderer>(true).ToList();
				destroyedMeshes.RemoveAll(x =>
					x.GetType() != typeof(MeshRenderer) && x.GetType() != typeof(SkinnedMeshRenderer));
				...
				if (destructible.replaceMaterials.Count > 0)
					destructible.replaceMaterials.RemoveAll(x =>
						x.SourceMaterial != null && !materialOptions.Contains(x.SourceMaterial.name));
				...
				if (destroyedMaterials.Count > 0)
				{
					...
					foreach (MaterialMapping mapping in destructible.replaceMaterials)
					{
						...
						int selectedMaterialIndex = mapping.SourceMaterial == null
							? 0
							: destroyedMaterials.IndexOf(mapping.SourceMaterial);
						...
						mapping.ReplacementMaterial =
							EditorGUILayout.ObjectField(mapping.ReplacementMaterial, typeof(Material), false) as
								Material;
						EditorGUILayout.EndHorizontal();
					}
				...
				else
				{
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					EditorGUILayout.LabelField("No materials found on prefab!", GUILayout.Width(200),
						GUILayout.Height(16));
					EditorGUILayout.EndHorizontal();
				}
				...
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.PropertyField(destroyedPrefabParent,
					new GUIContent("Assigned Parent:",
						"When destroyed, the destroyed prefab will spawn in as a child of this game object. If blank, the destroyed prefab will have no parent."));
				...
				List<Transform> debrisObjects =
					destructible.destroyedPrefab.GetComponentsInChildrenOnly<Transform>(true);
				if (debrisObjects.Count > 0)
				{
					EditorGUILayout.LabelField(new GUIContent("Debris to Re-Parent:",
						"Here you can assign debris pieces that will be re-parented to this game object's parent when destroyed. For example, if a character has a sword that gets destroyed, you might want to re-parent the broken handle back to the character's hand bone."));
					...
					if (destructible.debrisToReParentByName.Count > 0)
					{
						...
						for (int i = 0; i < destructible.debrisToReParentByName.Count; i++)
						{
							...
							int currentSelectedIndex =
								debrisOptions.FindIndex(m => m == destructible.debrisToReParentByName[i]);
							int selectedDebrisIndex =
								EditorGUILayout.Popup(currentSelectedIndex, debrisOptions.ToArray());
						...
						EditorGUILayout.BeginHorizontal();
						EditorGUILayout.LabelField("", GUILayout.Width(15));
						destructible.debrisToReParentIsKinematic = EditorGUILayout.Toggle(
							new GUIContent("Make Kinematic",
								"When FALSE, debris pieces that are reparented under this game object's parent will be affected by gravity/forces once the object is destroyed. Set to TRUE for things like a sword handle reparented to a rigged hand bone."),
							destructible.debrisToReParentIsKinematic);
						EditorGUILayout.EndHorizontal();
					}
				...
				if (destructibleChildren.Count > 0)
				{
					...
					if (destructible.childrenToReParentByName.Count > 0)
					{
						...
						for (int i = 0; i < destructible.childrenToReParentByName.Count; i++)
						{
							...
							EditorGUILayout.LabelField("Name", GUILayout.Width(50));
							int currentSelectedIndex =
								destructibleChildrenOptions.FindIndex(
									m => m == destructible.childrenToReParentByName[i]);
							int selectedChildIndex = EditorGUILayout.Popup(currentSelectedIndex,
								destructibleChildrenOptions.ToArray());
							if (selectedChildIndex >= 0 && selectedChildIndex < destructibleChildrenOptions.Count)
								destructible.childrenToReParentByName[i] =
									destructibleChildrenOptions[selectedChildIndex];
							else
					...
					// Add/Remove buttons
					bool showChildReparentAddButton =
						destructible.childrenToReParentByName.Count < destructibleChildrenOptions.Count;
					bool showChildReparentRemoveButton = destructible.childrenToReParentByName.Count > 0;
					CreateButtons(destructible.childrenToReParentByName, showChildReparentAddButton,
						showChildReparentRemoveButton);
				}
				...
				if (destructible.isDebrisChipAway)
				{
					...
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					chipAwayDebrisMass.floatValue =
						EditorGUILayout.FloatField("Debris Mass", chipAwayDebrisMass.floatValue);
					...
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					chipAwayDebrisDrag.floatValue =
						EditorGUILayout.FloatField("Debris Drag", chipAwayDebrisDrag.floatValue);
					...
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					chipAwayDebrisAngularDrag.floatValue = EditorGUILayout.FloatField("Debris Angular Drag",
						chipAwayDebrisAngularDrag.floatValue);
				...
				autoPoolDestroyedPrefab.boolValue = EditorGUILayout.Toggle(
					new GUIContent("Auto Pool",
						"If checked, the destroyed prefab for this object will be added to the object pool automatically for you at runtime."),
					autoPoolDestroyedPrefab.boolValue);
			...
			// FALLBACK PARTICLE
			EditorGUILayout.BeginHorizontal();
			useFallbackParticle.boolValue =
				EditorGUILayout.Toggle("", useFallbackParticle.boolValue, GUILayout.Width(15));
			GUIContent fallbackParticleContent = new GUIContent("Use Fallback Particle",
				"The particle effect used when this object is: (1) destroyed when there is no destroyed prefab assigned, (2) destroyed when a destroyed prefab is assigned but the DestructionManager's destroyed prefab limit has been reached, or (3) obliterated. If unchecked, the object will disappear when destroyed. If checked but unassigned, the default particle effect will be used.");
			...
			if (destructible.useFallbackParticle)
			{
				...
				if (destructible.fallbackParticle == null)
				{
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					GUIContent fallbackParticleMaterialContent = new GUIContent("Material Override:",
						"Here you can specify the material that will be used for the fallback particle effect. Leave blank to use the first material (index 0) on the Destructible object's mesh.");
					EditorGUILayout.LabelField(fallbackParticleMaterialContent, GUILayout.Width(105));
					EditorGUILayout.EndHorizontal();
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					fallbackParticleMaterial.objectReferenceValue =
						EditorGUILayout.ObjectField(destructible.fallbackParticleMaterial, typeof(Material), false) as
							Material;
				...
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.LabelField("", GUILayout.Width(15));
				destructible.centerPointOverride = EditorGUILayout.Vector3Field(
					new GUIContent("Position Override",
						"Here you can override the position the particle effect will spawn. This is particularly useful for Static objects, since Destructible uses the transform's position as the particle spawn point, which may not always be the center of the mesh."),
					centerPointOverride.vector3Value);
				EditorGUILayout.EndHorizontal();
			}
			...
			List<string> childrenOptions =
				children.Select(x => String.Format("{0} [{1}]", x.name, x.GetInstanceID())).ToList();
			...
			if (children.Count > 0)
			{
				...
				EditorGUILayout.Separator();
				EditorGUILayout.LabelField(new GUIContent("Un-Parent Children When Destroyed:",
					"These child objects will be un-parented from the destructible object when it is destroyed."));
				...
				if (rigidbodyFound)
				{
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("", GUILayout.Width(15));
					destructible.disableKinematicOnUparentedChildren = EditorGUILayout.Toggle(
						new GUIContent("Turn off Kinematic",
							"When TRUE, kinematic rigidbodies on unparented children will be affected by gravity/forces once the object is destroyed."),
						destructible.disableKinematicOnUparentedChildren);
					EditorGUILayout.EndHorizontal();
				}
				else
					destructible.disableKinematicOnUparentedChildren =
						true; // default to TRUE when future children are added to unparent.
