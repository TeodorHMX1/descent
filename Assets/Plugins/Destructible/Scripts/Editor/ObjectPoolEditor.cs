...
namespace Destructible
{
	...
	public class ObjectPoolEditor : Editor
	{
		...
		private readonly string[]
			...
			{
				":|:"
			}; // The complex delimiter for loading/saving Object Pool to file. Should be something you won't use in your prefab names.
		...
		public override void OnInspectorGUI()
		{
			...
			if (changeEntries.Count > 0)
			{
				...
				foreach (PoolEntry entry in changeEntries)
				{
					...
					if (entry != null && entry.Prefab != null)
					{
						...
						{
							Debug.LogWarning("Prefab \"" + entry.Prefab.name +
											 "\" already exists in Object Pool (item #" +
											 (previouslyUsedNames.IndexOf(entry.Prefab.name) + 1) + ").");
			...
			{
				string path = EditorUtility.OpenFilePanel("Import Object Pool Save File",
					SceneManager.GetActiveScene().path.SceneFolder(), "txt");
				if (path.Length != 0 && File.Exists(path))
				{
					string[] lines = File.ReadAllLines(path);
					if (EditorUtility.DisplayDialog("Add Items to Object Pool?",
						"Are you sure you want to import " + lines.Length + " items into the Object Pool from " +
						Path.GetFileName(path) + "?", "Add Items", "Cancel"))
					{
						int itemsModified = 0;
						int itemsAdded = 0;
						for (int i = 0; i < lines.Length; i++)
						{
							...
							{
								changeEntries.Add(new PoolEntry
									{Prefab = prefab, Count = count, OnlyPooled = onlyPooled});
								itemsAdded++;
							}
						}

						Debug.Log(String.Format(
							"{0} imported into Object Pool. Results: {1} new items added, {2} existing items updated.",
							Path.GetFileName(path), itemsAdded, itemsModified));
					}
				}
			}
			...
			if (GUILayout.Button("Clear", EditorStyles.toolbarButton, GUILayout.Width(70)) &&
				EditorUtility.DisplayDialog("Clear Object Pool?",
					"Are you sure you want to remove all objects from the object pool?", "Clear", "Cancel"))
			{
			...
			{
				string saveFilePath = EditorUtility.SaveFilePanel("Save Object File To",
					SceneManager.GetActiveScene().path.SceneFolder(),
					SceneManager.GetActiveScene().name.Replace(" ", "") + "ObjectPool.txt", "txt");
				...
				if (saveFilePath.Length != 0)
				{
					if (objectPool.prefabsToPool != null && objectPool.prefabsToPool.Count > 0)
					{
						...
						{
							string assetPath =
								AssetDatabase.GetAssetPath(objectPool.prefabsToPool[i].Prefab.GetInstanceID());
							string assetId = AssetDatabase.AssetPathToGUID(assetPath);
							lines[i] = String.Format("{1}{0}{2}{0}{3}{0}{4}", string.Join("", delimiter),
								objectPool.prefabsToPool[i].Prefab.name, assetId, objectPool.prefabsToPool[i].Count,
								objectPool.prefabsToPool[i].OnlyPooled);
						}
