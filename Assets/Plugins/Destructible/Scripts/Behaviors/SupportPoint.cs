...
namespace Destructible
{
	...
	public class SupportPoint : MonoBehaviour
	{
		...
		void Start()
		{
			...
			{
				Debug.Log("[" + name +
						  "] has no parent. Support points are designed to be children of objects that have attached colliders.");
				_canSupport = false;
			}
			else if (transform.parent.GetComponent<Collider>() == null ||
					 !transform.parent.GetComponent<Collider>().enabled)
			{
				Debug.Log("[" + transform.parent.name +
						  "] has a support point but no enabled collider. Support points only work on objects with colliders.");
				_canSupport = false;
			}
			else if (transform.parent.GetComponent<Collider>().attachedRigidbody == null)
			{
				Debug.Log("[" + transform.parent.name +
						  "] has a support point but no attached rigidbody. Support points only work on objects with rigidbodies.");
				_canSupport = false;
			}

			if (_canSupport)
			{
				Ray ray = new Ray(transform.position - (transform.forward * 0.025f),
					transform.forward); // need to start the ray behind the transform a little
				...
				for (int i = 0; i < hitInfo.Length; i++)
				{
					...
					Vector3 angleAxis =
						transform.parent.transform.InverseTransformDirection(
							transform.TransformDirection(Vector3.forward));

					// Add a stiff joint for support.
					transform.parent.GetComponent<Collider>().attachedRigidbody.gameObject.AddStiffJoint(
						hitInfo[i].collider.attachedRigidbody,
