...
namespace Destructible
{
	public class ActivateDestructibles : MonoBehaviour
	{
		...
		private void OnTriggerExit(Collider other)
		{
			...
			if (destructible.enabled && !destructible.isTerrainTree && _destructionManager.autoDeactivateDestructibles
			) // deactivate non-terrain trees
				destructible.shouldDeactivate = true;
			else if (destructible.enabled && destructible.isTerrainTree &&
					 _destructionManager.autoDeactivateDestructibleTerrainObjects) // deactivate terrain trees
				destructible.shouldDeactivate = true;
		}
	}
}