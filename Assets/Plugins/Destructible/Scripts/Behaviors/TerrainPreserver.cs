		...
		public void Awake()
		{
			...
			if (terrainDataBkp != null)
			{
				int option = EditorUtility.DisplayDialogComplex("Restore Terrain Trees from Backup?",
					"The Unity Editor may not have shut down correctly last time, which could mean destructible terrain trees were removed from terrain [" +
					terrain.terrainData.name + "]. Fortunately, there is a backup.\n\n" +
